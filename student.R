setwd("C:/Users/Architect_shwet/Desktop/Student-alcohol-consumption-and-grade-prediction-master/Student-alcohol-consumption-and-grade-prediction-master")
...
#No missing values
sum(is.na(stu))
missing_values <- stu %>% summarize_each(funs(sum(is.na(.))/n()))
...
#Gives the percentage of each category of sex
prop.table(table(stu$sex))

#Gives the percentage of each category of age
prop.table(table(stu$age))
...
stu %>% ggplot(aes(x= sex, y= G1) )+ 
  geom_jitter(color='grey', alpha=0.2) + 
  geom_violin(fill='red', alpha=0.7) +
  ggtitle('sex vs G1')

stu %>% ggplot(aes(x= sex, y= G2) )+ 
  geom_jitter(color='grey', alpha=0.2) + 
  geom_violin(fill='red', alpha=0.7) +
  ggtitle('sex vs G1')

stu %>% ggplot(aes(x= sex, y= G3) )+ 
  geom_jitter(color='grey', alpha=0.2) + 
  geom_violin(fill='red', alpha=0.7) +
  ggtitle('sex vs G3')

stu %>% ggplot(aes(x= sex, y= studytime) )+ 
  geom_jitter(color='grey', alpha=0.2) + 
  geom_violin(fill='red', alpha=0.7) +
  ggtitle('sex vs studytime')

stu %>% ggplot(aes(x= sex, y= traveltime) )+ 
  geom_jitter(color='grey', alpha=0.2) + 
  geom_violin(fill='red', alpha=0.7) +
  ggtitle('sex vs traveltime')

prop.table(table(stu$sex, stu$traveltime))

#plotting histogram of age and scaling by coloring by sex
ggplot(aes(x =age, fill=sex) , data = na.omit(stu)) +
  geom_histogram(binwidth=.1) +
  scale_color_brewer(type='div')
#max students belonging to age 16,17,18  & 15
#has more no of females in the sample

# Use ggplot2 to visualize the relationship between age & sex
ggplot(na.omit(stu), aes(x = age, fill = sex)) +
...
# Use ggplot2 to visualize the relationship between age & sex
ggplot(na.omit(stu), aes(x = age, fill = internet)) +
  geom_bar(stat='count', position='dodge') +
  scale_x_continuous(breaks=c(14:23)) +
  labs(x = 'age') +
  theme_bw()

#Freq-polygon for age vs sex
ggplot(aes(x=age,color=sex), data  = na.omit(stu)) + 
  geom_freqpoly(binwidth=1) + 
  scale_x_continuous(limits=c(14,23), breaks=seq(15,20,1))



#mosaic plot representation between sex and school
mosaicplot(table(stu$sex, stu$age), main='sex vs age', shade=TRUE)

#mosaic plot representation between sex and school
mosaicplot(table(stu$sex, stu$school), main='sex vs school', shade=TRUE)


col_pal <- "Set1"

stu %>% 
  ggplot(aes(x=sex, fill=address, color=address)) + geom_line(stat="density", size=1.2) + 
  theme_bw()+scale_fill_brewer(palette=col_pal)+scale_color_brewer(palette=col_pal)

stu %>% 
  ggplot(aes(x=sex, fill=famsize, color=famsize)) + geom_line(stat="density", size=1.2) + 
  theme_bw()+scale_fill_brewer(palette=col_pal)+scale_color_brewer(palette=col_pal)


stu %>% ggplot(aes(x=Fedu, fill=school, color=school)) + geom_line(stat="density", size=1.2) + 
      theme_bw()+scale_fill_brewer(palette=col_pal)+scale_color_brewer(palette=col_pal)
prop.table(table(stu$school))

stu %>% 
  ggplot(aes(x=Fedu, fill=famsize, color=famsize)) + geom_line(stat="density", size=1.2) + 
  theme_bw()+scale_fill_brewer(palette=col_pal)+scale_color_brewer(palette=col_pal)

stu %>% 
  ggplot(aes(x=Fedu, fill=address, color=address)) + geom_line(stat="density", size=1.2) + 
  theme_bw()+scale_fill_brewer(palette=col_pal)+scale_color_brewer(palette=col_pal)
...
prop.table(table(stu$freetime, stu$absences))

ggplot(aes(x=freetime, y=absences), data= stu) + 
  geom_point(color='red', alpha=0.4) + 
  geom_smooth(method='lm', color='grey') +
  ggtitle('freetime vs absences')

#health vs absences

prop.table(table(stu$health))
#Most of the students have good health 

ggplot(stu, aes(x=health, y=absences)) +
  geom_point(color = 'red', size=2, shape=23, alpha =0.4)

ggplot(aes(x=health, y=absences), data= stu) + 
  geom_point(color='red', alpha=0.4) + 
  geom_smooth(method='lm', color='grey') +
  ggtitle('health vs absences')

#freetime vs walc

table(stu$Walc)

prop.table(table(stu$freetime,stu$Walc))

ggplot(aes(x=freetime, y=Walc), data= stu) + 
  geom_point(color='red', alpha=0.4) + 
  geom_smooth(method='lm', color='grey') +
  ggtitle('freetime vs walc')

# goout vs walc

ggplot(aes(x=goout, y=Walc), data= stu) + 
  geom_point(color='red', alpha=0.4) + 
  geom_smooth(method='lm', color='grey') +
  ggtitle('goout vs walc')

prop.table(table(stu$goout,stu$Walc))

ggplot(stu, aes(x = goout, fill = factor(Walc))) +
  geom_bar(stat='count', position='dodge') +
  scale_x_continuous(breaks=c(1:11)) +
  labs(x = 'goout vs walc') +
  theme_few()
